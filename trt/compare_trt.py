"""Compare the TensorRT-backed pipeline (trt/segment_all_trt.py) against
examples/fast_image_segmentation.py's fp16 + torch.compile pipeline over
examples/images, the same way examples/compare_segmentation.py compares the
fp16+compile pipeline against the fp32 eager baseline.
...
import fast_image_segmentation as fast  # noqa: E402
...
def main():
    ...
    fast_results, fast_warmup_s = run_all(
        ...
        fast.segment_all,
        image_paths,
        "fast: fp16 + compiled encoder/decoder (torch.compile)",
    )
    print()
    ...
    header = (
        f"\n{'image':35s} {'masks fast/trt':>15s}  {'ms fast/trt':>18s}  "
        f"{'speedup':>8s}  {'coverage IoU':>12s}"
    )
    print(header)
    total_fast_ms = total_trt_ms = 0.0
    for path in image_paths:
        ...
        intersection = np.logical_and(f_union, t_union).sum()
        union_px = np.logical_or(f_union, t_union).sum()
        iou = intersection / union_px if union_px else 1.0
        speedup = f["ms"] / t["ms"]

        print(
            f"{path.name:35s} {len(f['anns']):>6d} / {len(t['anns']):<6d}  "
            f"{f['ms']:>8.1f} / {t['ms']:<7.1f}  {speedup:>7.2f}x  {iou:>11.4f}"
        )

    print(f"\nwarmup:       fast(compile) {fast_warmup_s:6.1f}s   trt {trt_warmup_s:6.1f}s")
    print(
        f"segmentation: fast(compile) {total_fast_ms/1000:6.1f}s   trt {total_trt_ms/1000:6.1f}s  "
        f"({total_fast_ms/total_trt_ms:.2f}x overall)"
    )
